...
class config(object):
    ...
    def plot(self, *args):
        ...
        if self.varE.get():
            ...
            utils.make_plot(first_entry, second_entry, self.entry1, self.entry2, res, "Breakeven Plot Energy", self.path_to_output_directory, self.increment, self.mindays, self.maxdays, self.minyears, self.maxyears)
        if self.varC.get():
            res = chip_breakeven_IPC(config_dicts, True, self.increment)['upgradeDays']
            utils.make_plot(first_entry, second_entry, self.entry1, self.entry2, res, "Breakeven Plot Carbon", self.path_to_output_directory, self.increment, self.mindays, self.maxdays, self.minyears, self.maxyears)
    ...
    def launch_config(self):
        ...
        self.window.title("All Breakeven")
